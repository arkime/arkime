...
async function initializeParliament () {
  ...
  parliamentFile = await require(`${getConfig('parliament', 'file')}`);
  ...
  if (parliamentFile && (parliamentFile.version === undefined || parliamentFile.version < MIN_PARLIAMENT_VERSION)) {
    ...
    // do the upgrade
    const upgraded = await upgrade.upgrade(parliamentFile, ArkimeConfig, internals.parliamentName, issues);
  ...
  try { // create parliament in db if it doesn't exist (there was no json file to upgrade from)
