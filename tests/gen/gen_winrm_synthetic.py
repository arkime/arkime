#!/usr/bin/env python3
"""Generate winrm_synthetic.pcap covering WS-Management / WinRM HTTP traffic.

WinRM is plain HTTP on 5985 (HTTP) or 5986 (HTTPS). Every operation is a
POST /wsman with a SOAP envelope. PowerShell Remoting, evil-winrm, winrs,
and Windows Event Forwarding all hit this endpoint.

Sessions:
   1  IPv4 :5985 POST /wsman          -> winrm  (PSRP / enumerate)
   2  IPv4 :5985 POST /wsman          -> winrm  (PSRP / invoke shell)
   3  IPv6 :5985 POST /wsman          -> winrm  (Identify diagnostic)
   4  IPv4 :5985 POST /wsmania        -> (no tag; negative control)
"""
...
SOAP_ENUM = (b'<s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope"'
             b' xmlns:wsa="http://schemas.xmlsoap.org/ws/2004/08/addressing"'
             b' xmlns:wsman="http://schemas.dmtf.org/wbem/wsman/1/wsman.xsd">'
             b'<s:Header><wsa:Action>http://schemas.xmlsoap.org/ws/2004/09/enumeration/Enumerate</wsa:Action>'
             b'</s:Header><s:Body/></s:Envelope>')

SOAP_SHELL = (b'<s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope"'
              b' xmlns:rsp="http://schemas.microsoft.com/wbem/wsman/1/windows/shell">'
              b'<s:Header><wsa:Action>http://schemas.xmlsoap.org/ws/2004/09/transfer/Create</wsa:Action>'
              b'</s:Header><s:Body><rsp:Shell><rsp:InputStreams>stdin</rsp:InputStreams>'
              b'<rsp:OutputStreams>stdout stderr</rsp:OutputStreams></rsp:Shell></s:Body></s:Envelope>')

SOAP_IDENTIFY = (b'<s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope"'
                 b' xmlns:wsmid="http://schemas.dmtf.org/wbem/wsman/identity/1/wsmanidentity.xsd">'
                 b'<s:Header/><s:Body><wsmid:Identify/></s:Body></s:Envelope>')


def post(path, host, body):
...
SESSIONS = [
    (False, '10.0.3.1',  '10.0.3.2', 52001, 5985, post('/wsman',   'dc.example.com', SOAP_ENUM),    resp()),
    (False, '10.0.3.3',  '10.0.3.4', 52002, 5985, post('/wsman',   'dc.example.com', SOAP_SHELL),   resp()),
    (True,  '2001:db8::7', '2001:db8::8', 52003, 5985, post('/wsman',   'dc.example.com', SOAP_IDENTIFY), resp()),
    (False, '10.0.3.5',  '10.0.3.6', 52004, 5985, post('/wsmania', 'other.example.com', b'hello'),   resp()),
]


def main():
    outpath = sys.argv[1] if len(sys.argv) > 1 else 'pcap/winrm_synthetic.pcap'
