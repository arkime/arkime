...
# http
    ...
    my $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/test/" . $sdId->{data}->[0]->{id} . "/sessionDetail?line=false&ts=false&base=natural")->content;
    ok(bin2hex($sd) =~ /636f6c3a2038303a71756963.*08000000000002/, "encoding:natural");

    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/test/" . $sdId->{data}->[0]->{id} . "/sessionDetail?line=false&ts=false&base=ascii")->content;
    ok(bin2hex($sd) =~ /636f6c3a2038303a71756963.*08000000000002/, "encoding:ascii");

    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/test/" . $sdId->{data}->[0]->{id} . "/sessionDetail?line=false&ts=false&base=hex")->content;
    ok($sd =~ /636f 6c3a 2038 303a 7175 6963 0d0a 0d0a.*col:.80:quic.*0800 0000 0000 02ff/s, "encoding:hex");

    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/test/" . $sdId->{data}->[0]->{id} . "/sessionDetail?line=true&ts=false&base=hex")->content;
    ok($sd =~ /00000272:.*636f 6c3a 2038 303a 7175 6963 0d0a 0d0a.*col:.80:quic.*00000000:.*0800 0000 0000 02ff/s, "encoding:hex line:true");

# http gzip:true
    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/test/" . $sdId->{data}->[0]->{id} . "/sessionDetail?line=false&ts=false&base=natural&gzip=true")->content;
    ok($sd =~ /col:.80:quic.*&lt;\?xml version=&quot;1.0&quot;\?&gt;/s, "encoding:natural gzip:true");

    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/test/" . $sdId->{data}->[0]->{id} . "/sessionDetail?line=false&ts=false&base=ascii&gzip=true")->content;
    ok($sd =~ /col:.80:quic.*&lt;\?xml version=&quot;1.0&quot;\?&gt;/s, "encoding:ascii gzip:true");

    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/test/" . $sdId->{data}->[0]->{id} . "/sessionDetail?line=false&ts=false&base=hex&gzip=true")->content;
    ...
    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/test/" . $sdId->{data}->[0]->{id} . "/sessionDetail?line=true&ts=false&base=hex&gzip=true")->content;
...
# http image:true
    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/test/" . $sdId->{data}->[0]->{id} . "/sessionDetail?line=false&ts=false&base=natural&image=true")->content;
    my $id = $sdId->{data}->[0]->{id};
    ok($sd =~ m{col:.80:quic.*\Q$id\E/body/file/1/file1.pellet}s, "encoding:natural image:true");

    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/test/" . $sdId->{data}->[0]->{id} . "/sessionDetail?line=false&ts=false&base=ascii&image=true")->content;
    ok($sd =~ m{col:.80:quic.*\Q$id\E/body/file/1/file1.pellet}s, "encoding:ascii image:true");

    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/test/" . $sdId->{data}->[0]->{id} . "/sessionDetail?line=false&ts=false&base=hex&image=true")->content;
    ok($sd =~ m{636f 6c3a 2038 303a 7175 6963 0d0a 0d0a.*col:.80:quic.*\Q$id\E/body/file/1/file1.pellet}s, "encoding:hex image:true");

    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/test/" . $sdId->{data}->[0]->{id} . "/sessionDetail?line=true&ts=false&base=hex&image=true")->content;
    ok($sd =~ m{00000272:.*636f 6c3a 2038 303a 7175 6963 0d0a 0d0a.*col:.80:quic.*\Q$id\E/body/file/1/file1.pellet}s, "encoding:hex line:true image:true");
