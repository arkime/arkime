...
# new /detail api
    ...
    my $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/api/session/test/$id/detail")->content;
...
# http
    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/api/session/test/$id/packets?line=false&ts=false&base=natural")->content;
    ok(bin2hex($sd) =~ /636f6c3a2038303a71756963.*08000000000002/, "encoding:natural");

    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/api/session/test/$id/packets?line=false&ts=false&base=ascii")->content;
    ok(bin2hex($sd) =~ /636f6c3a2038303a71756963.*08000000000002/, "encoding:ascii");

    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/api/session/test/$id/packets?line=false&ts=false&base=hex")->content;
...
# http gzip:true
    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/api/session/test/$id/packets?line=false&ts=false&base=natural&gzip=true")->content;
    ok($sd =~ /col:.80:quic.*&lt;\?xml version=&quot;1.0&quot;\?&gt;/s, "encoding:natural gzip:true");

    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/api/session/test/$id/packets?line=false&ts=false&base=ascii&gzip=true")->content;
    ok($sd =~ /col:.80:quic.*&lt;\?xml version=&quot;1.0&quot;\?&gt;/s, "encoding:ascii gzip:true");

    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/api/session/test/$id/packets?line=false&ts=false&base=hex&gzip=true")->content;
...
# http image:true
    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/api/session/test/$id/packets?line=false&ts=false&base=natural&image=true")->content;
    ok($sd =~ m{col:.80:quic.*test/\Q$encodedId\E/body/file/1/crossdomain.xml.pellet}s, "encoding:natural image:true");

    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/api/session/test/$id/packets?line=false&ts=false&base=ascii&image=true")->content;
    ok($sd =~ m{col:.80:quic.*test/\Q$encodedId\E/body/file/1/crossdomain.xml.pellet}s, "encoding:ascii image:true");

    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/api/session/test/$id/packets?line=false&ts=false&base=hex&image=true")->content;
    ...
    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/api/session/test/$id/packets?line=true&ts=false&base=hex&image=true")->content;
...
# smtp
    ...
    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/api/session/test/$id/packets?line=false&ts=false&base=natural&image=false")->content;
...
# bodyHash in session
    ...
    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/api/session/test/$id/bodyhash/87b94c6c763d2ae6e47cfb9c35e6d54d8a84bf125cbc9430082a9f1b3e592bec")->content;
    is($sd, "Username=Qggh&Passwd=Bhhhh&Section=Auuu", "Right sha256");

    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/api/session/test/$id/bodyhash/a4a7b8b5eaf4c2cf356b1052133c6cdb")->content;
    is($sd, "Username=Q&Passwd=B&Section=A", "Right md5");

    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/api/session/test/$id/bodyhash/87b94c6c763d2ae6e47cfb9c35e6d54d8a84bf125cbc9430082a")->content;
    is($sd, "No match", "No Match");

# Global bodyHash
    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/api/sessions/bodyhash/87b94c6c763d2ae6e47cfb9c35e6d54d8a84bf125cbc9430082a9f1b3e592bec?date=-1&expression=http.sha256=87b94c6c763d2ae6e47cfb9c35e6d54d8a84bf125cbc9430082a9f1b3e592bec")->content;
    is($sd, "Username=Qggh&Passwd=Bhhhh&Section=Auuu", "Global Right sha256");

    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/api/sessions/bodyhash/a4a7b8b5eaf4c2cf356b1052133c6cdb?date=-1&expression=http.md5=a4a7b8b5eaf4c2cf356b1052133c6cdb")->content;
    is($sd, "Username=Q&Passwd=B&Section=A", "Global Right md5");

    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/api/sessions/bodyhash/a4a7b8b5eaf4c2cf356b1052133c6cdb?date=-1&expression=http.md5=1")->content;
    is($sd, "No Match Found", "No Match Found");

    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/api/sessions/bodyhash/a4a7b8b5eaf4c2cf356b105c6cdb?date=-1&expression=http.md5=a4a7b8b5eaf4c2cf356b1052133c6cdb")->content;
    is($sd, "No match", "No Match");

# ipv6/4 port separators
    $sdId = viewerGet("/sessions.json?date=-1&expression=" . uri_escape("file=$pwd/v6.pcap"));
    $id = $sdId->{data}->[0]->{id};
    $sd = $MolochTest::userAgent->get("http://$MolochTest::host:8123/api/session/test/$id/detail")->content;
