...
my ($param, $multi) = @_;
    ...
    my $txt;
    if ($multi) {
        $txt = $MolochTest::userAgent->get("http://$MolochTest::host:8125/multiunique.txt?$param")->content;
    } else {
        $txt = $MolochTest::userAgent->get("http://$MolochTest::host:8123/multiunique.txt?$param")->content;
    }
    my @lines = split(/\n/, $txt);
...
$mtxt = get("date=-1&exp=tags&expression=$files&counts=1", 1);
eq_or_diff($txt,
"byhost2, 7
byip1, 1
byip2, 1
...
$mtxt = get("date=-1&exp=http.hasheader.src&expression=$files&counts=1", 1);
...
$mtxt = get("date=-1&exp=http.md5&expression=$files", 1);
eq_or_diff($txt,
...
$mtxt = get("date=-1&exp=http.md5&expression=$files&counts=1", 1);
eq_or_diff($txt,
...
$mtxt = get("date=-1&exp=http.uri&expression=$files&counts=0", 1);
eq_or_diff($txt,
...
$mtxt = get("date=-1&exp=http.uri&expression=$files&counts=1", 1);
eq_or_diff($txt,
...
$mtxt = get("date=-1&exp=http.user-agent&expression=$files&counts=0", 1);
eq_or_diff($txt,
...
$mtxt = get("date=-1&exp=ip.src,port.src&expression=$files&counts=1", 1);
eq_or_diff($txt,
"10.0.0.1, 1637, 1
10.0.0.1, 54263, 1
10.0.0.2, 53709, 1
