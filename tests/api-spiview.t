...
# bigendian pcap file tests
    my $json = viewerGet("/spiview.json?interval=hour&map=true&date=-1&facets=1&spi=source.ip,destination.ip,ipProtocol,fileand&expression=" . uri_escape("file=$pwd/bigendian.pcap"));
    my $pjson = viewerPost("/api/spiview", '{"map":true, "date":-1, "facets":1, "spi":"source.ip,destination.ip,ipProtocol,fileand", "expression":"file=' . $pwd . '/bigendian.pcap"}');
    my $mjson = multiGet("/spiview.json?interval=hour&map=true&date=-1&facets=1&spi=source.ip,destination.ip,ipProtocol,fileand&expression=" . uri_escape("file=$pwd/bigendian.pcap"));
    my $djson = multiGet("/spiview.json?interval=hour&map=true&startTime=1332734457&stopTime=1389743152&facets=1&spi=source.ip,destination.ip,ipProtocol,fileand&expression=" . uri_escape("file=$pwd/bigendian.pcap"));
...
# bigendian pcap file tests no facets
    $json = viewerGet("/spiview.json?date=-1&spi=source.ip,destination.ip,ipProtocol&expression=" . uri_escape("file=$pwd/bigendian.pcap"));
    $pjson = viewerPost("/api/spiview", '{"date":-1, "spi":"source.ip,destination.ip,ipProtocol", "expression":"file=' . $pwd . '/bigendian.pcap"}');
    $mjson = multiGet("/spiview.json?date=-1&spi=source.ip,destination.ip,ipProtocol&expression=" . uri_escape("file=$pwd/bigendian.pcap"));
...
# Check facets ALL
    $json = viewerGet("/spiview.json?interval=hour&map=true&date=-1&facets=1&spi=source.ip,destination.ip,ipProtocol,fileand,tags:5,http.requestHeader&expression=" . uri_escape("file=$pwd/bigendian.pcap|file=$pwd/socks-http-example.pcap|file=$pwd/bt-tcp.pcap"));
    $pjson = viewerPost("/api/spiview", '{"map":true, "date":-1, "facets":1, "spi":"source.ip,destination.ip,ipProtocol,fileand,tags:5,http.requestHeader", "expression":"file=' . $pwd . '/bigendian.pcap|file=' . $pwd .'/socks-http-example.pcap|file=' . $pwd .'/bt-tcp.pcap"}');
    $mjson = multiGet("/spiview.json?interval=hour&map=true&date=-1&facets=1&spi=source.ip,destination.ip,ipProtocol,fileand,tags:5,http.requestHeader&expression=" . uri_escape("file=$pwd/bigendian.pcap|file=$pwd/socks-http-example.pcap|file=$pwd/bt-tcp.pcap"));
    $djson = viewerGet("/spiview.json?interval=hour&map=true&startTime=1332734457&stopTime=1482563001&facets=1&spi=source.ip,destination.ip,ipProtocol,fileand,tags:5,http.requestHeader&expression=" . uri_escape("file=$pwd/bigendian.pcap|file=$pwd/socks-http-example.pcap|file=$pwd/bt-tcp.pcap"));
    ...
    eq_or_diff($json->{spi}->{tags}, from_json('{"doc_count_error_upper_bound": 0, "sum_other_doc_count": 12,
            "buckets":[{"doc_count":3, "key":"byhost2"},{"doc_count":3, "key":"domainwise"},{"doc_count":3, "key":"hosttaggertest1"},{"doc_count":3, "key":"hosttaggertest2"},{"doc_count":5, "key":"zeek:intel"}]}'), "ALL tags");
