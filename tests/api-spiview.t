...
# Check facets ALL
    $json = viewerGet("/spiview.json?date=-1&facets=1&spi=a1,a2,pr,fileand,ta:5,hh1&expression=" . uri_escape("file=$pwd/bigendian.pcap|file=$pwd/socks-http-example.pcap|file=$pwd/bt-tcp.pcap"));
    $mjson = multiGet("/spiview.json?date=-1&facets=1&spi=a1,a2,pr,fileand,ta:5,hh1&expression=" . uri_escape("file=$pwd/bigendian.pcap|file=$pwd/socks-http-example.pcap|file=$pwd/bt-tcp.pcap"));
    ...
    my @items = sort({$a->{key} cmp $b->{key}} @{$json->{spi}->{ta}->{buckets}});
    $json->{spi}->{ta}->{buckets} = \@items;
    @items = sort({$a->{key} cmp $b->{key}} @{$mjson->{spi}->{ta}->{buckets}});
    $mjson->{spi}->{ta}->{buckets} = \@items;

    eq_or_diff($json->{spi}->{ta}, from_json('{"doc_count_error_upper_bound": 0, "sum_other_doc_count": 6,
