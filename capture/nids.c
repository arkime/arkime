...
void moloch_nids_file_create()
{
    ...
    dumperFd = open(dumperFileName,  O_LARGEFILE | O_NOATIME | O_WRONLY | O_NONBLOCK | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH);
    if (dumperFd < 0) {
...
void moloch_nids_cb_ip(struct ip *packet, int len)
{
    ...
    if (!session) {
        ...
        switch (packet->ip_p) {
        case IPPROTO_TCP:
            /* If antiSynDrop option is set to true, then capture will assume that if the syn-ack packet was captured first then the syn probably got dropped.*/
...
void moloch_nids_exit() {
    ...
    if (!config.dryRun && config.copyPcap) {
        ...
        while (DLL_COUNT(mo_, &outputQ) > 0) {
            ...
            int len = write(out->fd, out->buf+out->pos, out->max - out->pos);
            if (len < 0) {
                LOG("ERROR - Write failed %d with %d %d\n", out->fd, len, errno);
                exit (0);
            }
            out->pos += len;
            if (out->pos < out->max) {
