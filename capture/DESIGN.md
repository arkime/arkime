...
When those data structures need to be updated we create a new one and replace the old one, which is schedule to be freed at a later time so any curernt readers don't crash.
...
## moloch-stats
Simple thread that just calculates all the stats occasionally.
...
## moloch-simple
A single thread that is responsible for writing out to disk the completed buffers.
